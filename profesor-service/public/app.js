  ...
  let calificaciones = [];
    ...
    async function initializePortal() {
        ...
        try {
            ...
            // LLAMAR A cargarGrupos() AQUÍ, DESPUÉS DE LA VALIDACIÓN
            cargarGrupos();

        } catch (error) {
            console.error("Error de validación:", error.message);
            logout(); // Si algo falla, esto te redirige
        }
    }
    ...
    async function cargarGrupos() {
        // Tu función para cargar grupos no necesita cambios
        try {
            ...
            gruposSelect.innerHTML = '<option value="">Mis Grupos</option>';
...
async function cargarAlumnos(grupoId) {
    ...
    if (!alumnos || alumnos.length === 0) {
      alumnosTableBody.innerHTML = '<tr><td colspan="8">No hay alumnos en este grupo.</td></tr>';
      return;
    }

    alumnos.forEach((alumno) => {
      const calif = calificaciones.find(c => c.matricula === alumno.matricula) || {};
      ...
      row.innerHTML = `
        ...
        <td>${alumno.nombre}</td>
        <td><input type="number" class="calif-input" data-parcial="1" value="${calif.parcial1 || ''}" min="0" max="10" step="0.1"></td>
        <td><input type="number" class="calif-input" data-parcial="2" value="${calif.parcial2 || ''}" min="0" max="10" step="0.1"></td>
        <td><input type="number" class="calif-input" data-parcial="3" value="${calif.parcial3 || ''}" min="0" max="10" step="0.1"></td>
  ...
  async function guardarCalificaciones(matricula) {
    ...
    try {
      ...
      if (estatus !== '-') {
          estatusCell.classList.add(final >= 6 ? 'status-aprobado' : 'status-reprobado');
      }
  ...
  alumnosTableBody.addEventListener('click', (event) => {
    ...
    // --- Sincronización de botones con funciones ---
